...
Ported near-verbatim from powerwave's app/providers/comtrade/comtrade_provider.py
(commit 3156392) -- only the import paths were adapted (app.models ->
app.domain, app.providers.base.* -> app.providers.base). The parsing
algorithm, scaling, timestamp handling, and error behaviour are
intentionally unchanged: this migration preserves verified powerwave
engineering behaviour rather than re-deriving it (see
docs/project-memory/MIGRATION_PLAN.md Sec 15, "Preserve behaviour vs
improve behaviour", and docs/project-memory/POWERWAVE_DISCOVERY.md).
...
app.services.import_service for how oruxa_powerwave stages an ephemeral
temporary directory to satisfy that requirement without persisting the
uploaded files.
"""
...
from app.providers.base import BaseProvider, ProviderLoadError
...
def _parse_binary_dat(
    ...
    row_size = 8 + 2 * cfg.n_analog + 2 * n_dwords
    ...
    if cfg.n_analog > 0:
        analog_raw = rows["ana"].astype(np.float64)
    else:
        analog_raw = np.empty((n_samples, 0), dtype=np.float64)

    if n_dwords > 0:
        digital_words = np.array(rows["dw"])
    else:
...
def _apply_analog_scaling(
    ...
    a_vec = np.array([d.a for d in analog_defs], dtype=np.float64)
    b_vec = np.array([d.b for d in analog_defs], dtype=np.float64)
...
def _extract_digital_channels(
    ...
    word_indices = ch_indices // 16
    bit_indices = ch_indices % 16
