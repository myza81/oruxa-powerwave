...
## DEC-075 — Data Preview shows a read-only, derived "Configured Time" column once the Time Axis is resolved, using the same standardized representation and normalization semantics as cleaned export (DEC-074) and canonical conversion

Date: 2026-09-04
Status: Approved
Source: explicit project-owner UAT feedback ("Show the Resolved/
Configured Time Axis in Data Preview" enhancement), delivered as a
direct follow-on to DEC-074 (export the resolved Time Axis) — the
owner's own stated problem: an engineer can configure and resolve the
Time Axis, but the Data Preview kept showing only the original source
Date/Time columns, so there was no way to directly verify the exact
time values Powerwave will actually use before converting or
exporting.

Decision:

**Data Preview gains one VIRTUAL, derived, READ-ONLY "Configured Time"
column, shown as the FIRST preview column, once the current Time Axis
is resolved enough to derive a real value from.** "Resolved enough" is
the SAME shared eligibility check cleaned export's own gate already
uses (`app.domain.time_axis.is_time_axis_resolved()`) — a genuine,
usable reading (including one that was never explicitly confirmed,
per the confirmation-UX policy, or that also carries a data-quality
warning) counts; an unconfigured/unresolved/stale-role-reference Time
Axis, a `manual`/`unsupported` interpreter, an unconfirmed
reconstruction suggestion, or `sample_index` with no real interval/rate
does not — no column is shown at all in those cases, never one with
fabricated or empty-looking cells.

**Representation is identical to cleaned export's own** (DEC-074): one
standardized ISO-8601 timestamp per row for a resolved absolute
reading (header `Time`), or fixed 3-decimal-place seconds relative to
the dataset's TRUE first active row for every other resolved family
(header `Time (s)`) — never the current preview PAGE's own first row,
a critical guardrail: a later page (e.g. rows 201-400) must still show
values continuing from the true start (e.g. `4.000`, `4.020`, ...),
never resetting to `0.000`. Computed by re-calling the ALREADY-
CONFIRMED interpreter's own `build_preview_rows()` over the FULL active
region (matching canonical conversion's and cleaned export's own
approach, for the same reason those two already process the full
region rather than a bounded page: `sample_index`/repeated-timestamp
reconstruction compute their own values relative to whatever row window
they are given, so that window must be the true full active region for
"first" to mean the dataset's real first active row), through the SAME
shared `app.services.time_axis_normalization` module DEC-074 already
established — never a third, independently re-derived parsing/cadence
algorithm.

**Strictly read-only and virtual.** The column is never assignable a
role, never editable, never clearable, never excludable, and is never
counted in `column_count`/`column_labels`/`column_roles` or given a
real column index — a wrong derived value is corrected by changing the
Time Axis configuration, never by editing the derived cell itself. The
original source Date/Time columns remain fully visible and editable
alongside it, for direct comparison and troubleshooting. Computing it
never mutates the raw source, the `WorkingOverlay`, or the Time Axis
configuration itself.

**Refreshed live, never cached across a Time Axis or working-cell
change.** Saving or clearing the Time Axis configuration, changing a
resolved date order, or editing a source Time Axis cell all trigger an
immediate preview re-fetch so the derived column appears/updates/
disappears without requiring a full browser reload.

Reason:

Closes the direct verification gap DEC-074 left open: an engineer could
configure and export/convert a resolved Time Axis, but had no way to
SEE the resulting values before doing so — the cleaned export was the
only place the resolved timing was ever visible, after the fact. Data
Preview is the natural place to show it, since it is already where the
engineer reviews and edits the working dataset before either action.
Reusing DEC-074's own representation/normalization semantics exactly
(rather than inventing a second display convention) means an engineer
never has to reconcile two different-looking timelines for what is
provably the same interpretation.

Alternatives considered:

Computing the derived column purely in frontend JavaScript from the
raw source cells (rejected outright by the task's own explicit
guardrail — the backend's `time_axis_interpreters`/`time_axis_
normalization` modules are the ONLY place date-order/cadence/
reconstruction logic may live; a frontend reimplementation would be
exactly the "third, independently re-derived" algorithm this decision
explicitly forbids). Replacing the original source Date/Time columns
with the derived one in Data Preview (rejected — the task's own
explicit "do not replace or hide original source columns" instruction;
direct comparison against the source is the whole point of showing
both). Computing relative time per-page (anchored to that page's own
first row) to avoid a full-active-region computation on every preview
request (rejected outright — this is the exact defect the task's own
section M explicitly guards against: a later page must never appear to
restart at zero). Caching the full-active-region computation across
requests (rejected — every other Slice 6/9 readiness/issue computation
in this codebase already derives live, uncached, on every request; a
new caching layer here would be the one inconsistent exception, and the
computation is already bounded by the same single-pass-over-the-active-
region shape `app.services.readiness_service`'s own full-region scan
already performs on every preview/issues interaction).

Impact:

Backend: `app/domain/time_axis.py` gains `is_time_axis_resolved()`
(shared with, and DEC-074's `preparation_export_service._ensure_
exportable()` refactored to reuse, rather than keeping its own
duplicate inline checks). `app/services/time_axis_normalization.py`
gains `relative_seconds_with_anchor()` (a generalization of the
existing `relative_seconds()`, which becomes a thin wrapper over it).
`app/services/time_axis_service.py` gains `ConfiguredTimeValues`/
`build_configured_time_values()` (the full-active-region computation)
and `configured_time_for_preview_page()` (the page-narrowing wrapper
`GET .../rows` calls). `app/schemas/preparation_session.py` gains
`ConfiguredTimePreviewOut` and an additive `PreparationSourcePreviewOut.
configured_time` field, populated by `app/api/v1/preparation_sources.py`'s
own `get_preparation_source_rows()` route, layered on top of the
existing raw-preview response (`column_count` and friends remain
exactly as before — the derived column is never counted among them).
Frontend: `frontend/index.html`'s `wwDataPrepRenderTable()` renders the
derived column first when `preview.configured_time` is present (a
distinct `.ww-data-prep-configured-time-cell` style, a small "Derived"
badge, and a tooltip explaining it), and the Time Axis Save/Clear
handlers now also re-fetch the preview so it updates immediately. Test
suites cover the new domain predicate, the full-region computation
(every time family, the paging/anchor guardrail, row-exclusion
handling, state-refresh, and cross-feature consistency with cleaned
export and canonical conversion), and the new API field. No change to
canonical `DisturbanceRecord` conversion, cleaned export's own
behavior, readiness policy, or the three-role column model (DEC-073).

---

## DEC-076 — Cleaned export's manifest/provenance bundle is now optional; the default "Export Cleaned Data" action returns the cleaned CSV/XLSX directly, never a ZIP

Date: 2026-09-04
Status: Approved
Source: explicit project-owner UAT feedback ("Cleaned Export UX --
manifest/provenance should be optional" enhancement) -- a direct
owner-approved amendment to the export PACKAGING policy DEC-074
established (DEC-074's own Time-Axis-resolution/gating decisions are
unchanged and remain fully in force).

Decision:

**The default cleaned-export action now returns the cleaned CSV/XLSX
bytes directly -- never a ZIP, never a forced sidecar
`manifest.json`.** Provenance/traceability capability itself is not
removed: `export_preparation_source()` gains an explicit `mode`
(`EXPORT_MODE_DATA_ONLY` -- the new default; `EXPORT_MODE_WITH_
PROVENANCE` -- the original Slice 12/DEC-074 ZIP+manifest bundle,
unchanged) and the same choice is exposed on the API as
`POST .../export?include_manifest=true|false` (default `false`). Both
modes share identical gating (`_ensure_exportable()`, unchanged from
DEC-074) and identical cleaned-data construction, so they always
produce byte-identical cleaned data for the same working-overlay
revision -- `mode`/`include_manifest` only decides the RETURN SHAPE.

Frontend: the "Export Cleaned Data" button's default click now performs
the data-only export; a secondary, visually subordinate "Download with
manifest" action performs the with-provenance export. Neither the
manifest schema nor the manifest CONTENTS changed.

Reason:

Owner-reported UX problem: an ordinary engineer only wants the reusable
cleaned CSV/XLSX and should never be handed a `.zip` -- let alone be
expected to understand a sidecar `manifest.json` -- merely to get it.
DEC-074 never intended to force a ZIP+manifest on every download; it
simply hadn't yet separated "what a cleaned export contains" from "what
the default click returns." This decision makes that separation
explicit without weakening the underlying provenance capability, which
remains available on request.

Alternatives considered:

Removing manifest/provenance generation entirely (rejected outright --
explicit non-goal; the manifest's own engineering traceability,
including the DEC-074 `exported_time` section, remains valuable and is
still produced on request). A new/different manifest schema (rejected
-- explicit non-goal; the SAME manifest-building code path is reused
unchanged for `EXPORT_MODE_WITH_PROVENANCE`). Building two separate
export pipelines, one per mode (rejected -- both modes share the exact
same gating and cleaned-data-assembly code; only manifest construction
and final packaging are mode-conditional, so a second competing
pipeline would only risk the two modes silently disagreeing about what
"cleaned data" means). Always building the manifest even for the
data-only path "just in case" (rejected -- task's own efficiency note;
`EXPORT_MODE_DATA_ONLY` skips `_build_manifest()`/its own edited/
cleared-cell provenance counts and JSON serialization entirely, not
merely discarding the result after building it). A dropdown/split
"Export Cleaned Data ▾" button (the owner's own first-preference UI)
was evaluated against this codebase's existing button vocabulary (plain
`<button>`/`button.secondary`, no dropdown/menu component anywhere
else in the Data Preparation Workspace) and judged disproportionately
complex to introduce for one action; the owner's own documented
fallback -- a primary button plus a visually secondary text-link-style
second action -- was used instead.

Impact:

Backend: `app/services/preparation_export_service.py` -- new
`EXPORT_MODE_DATA_ONLY`/`EXPORT_MODE_WITH_PROVENANCE` constants,
`export_preparation_source()` gains a `mode` parameter (default
`EXPORT_MODE_DATA_ONLY`), manifest construction is now conditional on
`mode == EXPORT_MODE_WITH_PROVENANCE`, `ExportResult` can now carry the
real CSV/XLSX `Content-Type` directly (`text/csv` /
`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`)
instead of always `application/zip`. `app/api/v1/preparation_sources.py`
-- `POST .../export` gains an `include_manifest: bool = Query(False, ...)`
query parameter mapping to the two modes; the endpoint docstring
updated. No new error classes; gating (`export_not_ready`/
`export_requires_interval`/`export_unsupported_interpreter`/
`export_revision_changed`) is identical across both modes. Frontend:
`frontend/index.html` -- the export action markup gains a second
button, `#wwDataPrepExportWithProvenanceBtn` ("Download with manifest
(cleaned file + provenance)"), styled as a plain underlined text link
(new `.ww-data-prep-export-provenance-link` CSS, deliberately not a
bordered `button.secondary`, so it reads as subordinate to the primary
"Export Cleaned Data" button); `wwDataPrepExport()` gains an
`includeManifest` parameter (`false`/`true` wired to the two buttons'
own click handlers) and appends `?include_manifest=true` to the
request URL only for the provenance action; the real filename/
extension is always read from the response's own `Content-Disposition`
header (never hard-coded `.zip`) with mode-aware fallback names for the
rare missing-header case; `wwDataPrepRenderExportAction()` now drives
both buttons' shared enabled/disabled state. Tests:
`test_preparation_export_service.py` -- every pre-existing test now
calls `export_preparation_source(..., mode=EXPORT_MODE_WITH_PROVENANCE)`
explicitly (unchanged behavior/assertions); new `TestDataOnlyExport`
and `TestModeEquivalence` classes cover the new default mode and
cross-mode data equivalence. `test_preparation_sources_api.py` --
`TestExportApi` rewritten around the new default (data-only) response
shape; new `TestWithProvenanceExportApi` class covers the
`?include_manifest=true` opt-in (moved from the old `TestExportApi`
assertions, unchanged). Full backend suite: 2731 passed, 0 failed (the
same combined baseline DEC-075 already established -- this enhancement
added/rewrote tests without changing the total pass count materially,
since it mostly re-scoped existing assertions across two modes rather
than adding a large new surface). The committed browser smoke test
(COMTRADE) still passes unchanged; a throwaway (not committed)
live-browser Playwright UAT confirmed a CSV and an Excel source's
default export both download the cleaned file directly (correct
`Content-Type`, correct source-derived filename, never a ZIP),
"Download with manifest" downloads a real ZIP containing both the
cleaned file and `manifest.json`, and the two exports' cleaned data is
byte-identical -- all with zero console/page errors. No change to
DEC-074's own Time-Axis resolution/gating logic, `DisturbanceRecord`/
canonical conversion, DEC-075's Configured Time preview column, or
COMTRADE handling.
...
## DEC-092 — Overcurrent chart UX enhancement: a Pickup Multiple/Relay Current X-axis representation toggle, and independently-toggleable major/minor logarithmic grid controls

Date: 2026-09-13
Status: Approved — implemented.
Source: owner task specification ("Overcurrent chart UX enhancement
only... configurable major/minor logarithmic grid behavior... switchable
OC x-axis representation between Pickup Multiple and Relay Current"),
building directly on the existing adjustable-viewport/curve-boundary-
alignment work under DEC-090.

Decision:

**1. A compact X-axis representation toggle, default Pickup Multiple.**
`wwOvercurrentSetXAxisMode()` switches the chart between the existing
normalized `M = Irelay / Ipickup` representation (unchanged default) and
a new Relay Current representation (amperes), via the exact relationship
`Irelay = M * Ipickup` (`wwOvercurrentMultipleToRelayCurrent()`/
`wwOvercurrentRelayCurrentToMultiple()`, `null`-guarded, never
approximated). This is a **pure chart-display choice** — the IEC IDMT
calculation, TMS, pickup, CT conversion, RMS estimator, Playback
integration, and DEC-091's engineering-unit normalization are all
byte-for-byte unchanged (verified by the existing domain/service/API
test suites passing unmodified).

**2. Switching representations is frontend-only — zero backend
requests.** Reuses the already-fetched characteristic geometry
(`curveCache`) and the already-computed `latestResult` verbatim;
`wwOvercurrentOperatingPointX(result)` is the one new helper that picks
which already-computed field (`multiple_of_pickup` or
`relay_secondary_current`) becomes the chart's operating-point
coordinate — the existing clamping/edge-indicator/guide-line logic in
`wwOvercurrentRenderChart()` needed zero changes, since it was already
unit-agnostic (it only ever compares against the active viewport's own
bounds, whatever they mean).

**3. The exact curve-boundary solution stays authoritative in the M
domain; Relay Current mode transforms the exact intersections, never a
second numeric solve.** `wwOvercurrentVisibleCurveSegment()` (the
existing exact analytic viewport-boundary solver) is always evaluated
against an M-domain-equivalent viewport; in Relay Current mode this
means dividing the active (amp-denominated) viewport by the current
pickup first, then transforming the resulting exact M-domain points back
to amperes via `Irelay = M * Ipickup` — mathematically exact apart from
ordinary floating-point precision, per the task's own explicit
requirement.

**4. Separate X-range memory per representation; Y stays shared.**
`wwOvercurrentState.savedXRangeByMode` remembers each mode's own last X
range so switching back restores where the user left off, never an
unusable/mismatched range; `viewport.yMin/yMax` (operating time,
seconds) is never duplicated per mode, since it means the same physical
quantity in both. Relay Current's own absolute bounds/default are the
SAME `WW_OC_VIEWPORT_ABSOLUTE`/`WW_OC_VIEWPORT_DEFAULT` X multipliers
(0.1x/200x, 0.1x/100x pickup) scaled by the current pickup — never a
second, independently-chosen amp range (task's own "do not hard-code a
single current range" instruction). A new pickup-boundary reference
line (`.ww-oc-pickup-boundary`, fixed at M=1 or moving with pickup in
amps) makes the threshold visible on the chart for the first time in
either representation.

**5. Major gridlines remain always visible; minor gridlines are new and
independently toggleable per axis, both default OFF.** Two checkboxes
(`#wwOvercurrentMinorGridXCheckbox`/`#wwOvercurrentMinorGridYCheckbox`)
gate a new minor-tick layer using the classic "log-log graph paper"
1-2-5 subdivision convention for X (`wwOvercurrentGenerateMinorPow125Ticks()`
— 1.2/1.4/1.6/1.8 between 1 and 2, the owner's own explicit example,
widening per sub-interval at every decade) and the standard `2..9 x
10^decade` set for the pure-decade Y axis
(`wwOvercurrentGenerateMinorDecadeTicks()`) — deliberately unlabeled, so
the chart never gets cluttered. Toggling either checkbox is a pure
re-render, zero backend requests, identical discipline to the existing
viewport/zoom controls.

**6. Both features respond correctly to the actual current viewport —
default, zoomed, or Relay-Current-mode — never a hard-coded range.**
Tick/grid generation for both major and minor sets is always derived
from whatever `viewport.xMin/xMax`/`yMin/yMax` currently are, exactly
mirroring the existing major-tick generation's own dynamic design.

**7. Session-local state only, following the existing precedent.**
`xAxisMode`/`minorGridX`/`minorGridY`/`savedXRangeByMode` live on
`wwOvercurrentState` in-memory, exactly like `settings`/`viewport`/
`curveCache` already do — survive Playback/phase/TMS/pickup/viewport
changes, chart resize, and a Phasor↔Overcurrent switch; reset only via
`wwOvercurrentResetState()` (full analyzer state recreation on new
workspace/clear). No backend persistence was added, matching every
other Overcurrent chart preference's own documented boundary.

Reason: the owner's own stated goal was to make the chart legible in
whichever unit an engineer is actually thinking in (a relay's own pickup
setting is naturally read in amperes, while protection-grading work is
naturally read in multiples-of-pickup) and to let the log-scale grid be
as detailed or as clean as the task at hand needs, without ever
reintroducing a backend round-trip for a purely visual change or
disturbing the exact curve-boundary correctness DEC-090's own UAT
follow-ups already established.

Alternatives considered:
- Re-deriving the curve segment independently for each axis mode (a
  second, amp-domain analytic solver) — rejected: the IDMT equation's
  own boundary solve is naturally expressed in M; transforming the
  ALREADY-exact M-domain result via the known-exact `Irelay = M *
  Ipickup` relationship is both simpler and provably exact, with no risk
  of the two solvers drifting apart.
- A single global linear minor-tick spacing (e.g. a flat 0.2 step across
  the whole visible range) — rejected per the task's own explicit
  instruction; would not preserve logarithmic geometry at wider zoom
  levels, and does not match the classic log-log graph-paper convention
  engineers already expect from a TCC-style chart.
- Sharing one X viewport across both representations (numerically
  reinterpreting the same min/max under the new unit) — rejected: 0.1-100
  is a sensible Pickup Multiple range but a nonsensical Relay Current
  range at most real pickup settings; per-mode X-range memory avoids
  ever landing on an unusable range after a mode switch.

Impact: `frontend/index.html` only (chart-rendering/state/markup/CSS;
`wwOvercurrentState` new fields, `wwOvercurrentRenderChart()` extended
in place, `wwOvercurrentZoom()`/`wwOvercurrentResetViewport()`/
`wwOvercurrentApplyViewportFromInputs()` made mode-aware, new helper
functions, new toggle/checkbox markup and CSS). No backend files
changed — this is a pure frontend chart-display feature. Updated tests:
`backend/tests/test_frontend_overcurrent_analysis.py` (one pre-existing
test's literal assertion updated to match the new M-domain-conversion
call shape, ~27 new tests added across 9 new test classes);
`browser-tests/overcurrent_analysis.spec.js` (11 new Playwright
scenarios). Full backend regression, full frontend static suite, and
the full Playwright suite (112 scenarios) all pass — one unrelated,
pre-existing Phasor Playback-throttle test flaked under full-suite load
and passed cleanly in isolation, confirmed unrelated to this change (no
Phasor/backend files were touched). `git diff --check` clean. See
[OVERCURRENT_ANALYSIS.md](OVERCURRENT_ANALYSIS.md)'s own "Chart X-axis
representation toggle and minor grid controls" section for the complete
architecture record.
...
## DEC-096 — Impedance Locus v1, the THIRD Analysis-menu analyzer: apparent phase impedance measurement/visualization, explicitly not Distance Protection

Date: 2026-09-17
Status: Approved — implemented.
Source: owner task ("implements the first version of the Impedance Locus
analyzer... calculate apparent phase impedance from voltage/current
phasors and visualize the result on an R-X plane... This analyzer is
NOT Distance Protection").

**Decision.** Powerwave gains a third Analysis-menu analyzer, Impedance
Locus, activating the existing placeholder entry (`#wwAnalysisTypeImpedanceBtn`,
preserving analyzer order: Phasor, Overcurrent, Impedance Locus, Sequence
Components). Scope is strictly `Za = Va/Ia`, `Zb = Vb/Ib`, `Zc = Vc/Ic`
via direct phasor division (`R = |Z|cos(theta)`, `X = |Z|sin(theta)`,
`theta = theta_V - theta_I` on each phasor's own absolute angle) — all
four R-X quadrants valid, never clamped. Explicitly **not** Distance
...
(`Zab`/`Zbc`/`Zca`), fault classification, directional logic, load
encroachment, power swing detection, or trip-decision claims exist
anywhere in this slice — those are reserved for a future, separate
Distance Protection analyzer.

**Recording mode reuses the existing Phasor estimator, never a second
one.** `app.services.impedance_analysis_service.compute_impedance_
...
page already uses) and reads out whichever Voltage/Current role pair the
selected phase needs — zero duplicated FFT/DFT/RMS code, and automatic
inheritance of that function's own reference-frequency/timebase/
waveform-form guardrails. `app.domain.analysis_requirements` gained
`IMPEDANCE_VOLTAGE_PHASE_A/B/C`/`IMPEDANCE_CURRENT_PHASE_A/B/C` under
their own `analysis_kind="impedance"` (declared for registry
completeness/future reuse; the service itself resolves via
`compute_phasor_diagram()`, not a second `resolve_analysis_inputs()`
call).

**Low-current guardrail is numerical validity, not a relay pickup
threshold.** `app.domain.impedance.MIN_CURRENT_A = 1e-3` (1 mA) — below
this floor, `status="needs_configuration"`/`reason_code=
"current_too_small"`, never a fabricated huge/infinite `|Z|`. Chosen
because `|Z|` is already meaningless for plotting at 1 mA for any
realistic voltage, well before genuine floating-point instability. Not
claimed as, and never presented as, a protection-relay pickup/
sensitivity specification.

**Primary/Secondary basis is explicit and, for Recording mode, an
optimization the task's own golden example depends on.** `recording_
basis` (what the resolved channels already represent) and `impedance_
basis` (the desired OUTPUT) are independent; when they already match, no
VT/CT ratio is required at all — `Z = V/I` is returned directly (matches
the task's own basis-agnostic golden example). Manual mode mirrors
Manual Phasor's architecture instead: Voltage and Current each carry
their OWN independent basis + VT/PT or CT ratio (reusing `convert_
manual_magnitude_to_secondary()` verbatim — never a duplicated
conversion formula), always normalized to Secondary-canonical
internally before division, with `impedance_basis` then acting as a
THIRD, independent output basis for the result — proven by the task's
own required example (Voltage Primary, Current Secondary, Impedance
output Primary). The conversion formula (`Z_primary = Z_secondary *
(VT_ratio/CT_ratio)`) was verified, not blindly adopted, against this
codebase's own existing ratio convention (`convert_to_relay_secondary()`/
`convert_manual_magnitude_to_secondary()`'s own established direction) —
see [IMPEDANCE_LOCUS_ANALYSIS.md](IMPEDANCE_LOCUS_ANALYSIS.md)'s own
"Basis conversion formula" section for the full derivation.

**Za/Zb/Zc phase selection** is a shared `Phase [A|B|C]` control
(default A) — Recording mode resolves phase roles entirely through the
existing Engineering Context / durable canonical-phase-identity
machinery (never inferred from a channel name); Manual mode uses it only
to label the result (e.g. "Zb"), with zero Engineering Context
dependency.

**R-X plot uses one shared px-per-ohm factor for both axes** —
structurally impossible to set an independent X/Y graphical scale, since
both axes read the same `pxPerOhm` variable (`wwImpedanceRenderPlot()`).
Origin at R=0/X=0, +R right, -R left, +X up, -X down, axis labels `R (Ω)`/
`X (Ω)`. Viewport is automatic: `wwImpedanceNiceLimit()` picks the
smallest nice `1×/2×/5×/10×`-per-decade value `>= 1.3×` the largest
`|R|`/`|X|` currently relevant, giving the point(s) headroom without an
oversized fixed range.

**Manual mode shows exactly one point** (dashed-ring marker, mirroring
Overcurrent's `.ww-oc-manual-marker` convention) — never a synthesized
locus/history from one manual value.

**Recording mode adds a genuine locus/trajectory, per the analyzer's own
name.** A new `GET .../impedance-locus` endpoint samples up to
`point_count` (default 120, hard-capped at `MAX_LOCUS_POINTS=300`)
evenly-spaced Impedance calculations across the active Time Group's own
full extent — each an independent, deterministic call (never built by
accumulating animation-frame samples during Playback, and never
Playback-speed-dependent). The frontend fetches this ONLY on a context/
phase/settings/time-range change (`wwImpedanceMaybeFetchLocus()`'s own
signature-based short-circuit, mirroring the shared Related Waveforms
panel's identical convention) — verified by a static test that the
Playback-tick handler itself never calls the locus fetch. The live
current-point fetch (`GET .../impedance`) remains a SEPARATE, throttled
(~10 Hz), Playback-synchronized call, identical in shape to Phasor's/
Overcurrent's own current-value fetch — no analyzer-specific timer
exists.
...
verbatim, the identical three-region markup separation (always-visible
Input Source toggle / Recording-only section / always-visible Body),
implemented from day one (never the flawed intermediate coupling
Overcurrent briefly shipped and later corrected). See
[ANALYSIS_INPUT_SOURCE.md](ANALYSIS_INPUT_SOURCE.md) for the pattern
this reuses and
[IMPEDANCE_LOCUS_ANALYSIS.md](IMPEDANCE_LOCUS_ANALYSIS.md) for the full
architecture record.

**Reusable foundation for a future Distance Protection analyzer.**
`app.domain.impedance` (the pure calculation engine:
`compute_impedance_point()`/`convert_impedance_basis()`) never imports or
references any zone/characteristic/fault-loop concept — a future
Distance Protection analyzer is expected to sit alongside it (its own
domain/service/schema/endpoint modules, own `analysis_kind="distance"`
requirement constants), consuming `ImpedancePoint` as a building block,
never requiring this module to change. Explicitly not built this slice.

**API.** `GET .../engineering-contexts/{id}/impedance` (selected-time,
one phase), `GET .../engineering-contexts/{id}/impedance-locus` (the
static trajectory), `GET .../impedance-manual` (workspace-scoped, never
context-nested) — same nested/selected-time/never-persisted router
convention Phasor's/Overcurrent's own endpoints already established, in
the same `app/api/v1/engineering_contexts.py` file.

**Tests.** `backend/tests/test_impedance_domain.py` (33 tests — golden
worked example, all-four-quadrant geometry, low-current guardrail,
basis-conversion formula matrix including the equal-VT/CT-ratio sanity
property, Manual per-quantity/per-family isolation),
`backend/tests/test_impedance_analysis_api.py` (16 tests — real
three-phase ASCII-COMTRADE-upload golden/phase-B/phase-C/basis-
conversion/low-current/locus/manual scenarios via real HTTP, zero prior
setup for Manual), `backend/tests/test_frontend_impedance_analysis.py`
(23 structural tests — markup separation, shared-constant reuse, locus-
never-per-tick, equal-scale-plot invariant, no protection-zone
identifiers). Two pre-existing static tests
(`test_frontend_phasor_analysis.py::TestSharedAnalysisContextConsumers`,
`test_frontend_playback.py::TestSharedAnalysisPlaybackStyling`) were
updated for the THIRD registered consumer/mounted transport, mirroring
the exact precedent Overcurrent's own addition already set for these
same two tests; `test_analysis_requirements.py`'s own registry-count
assertions were updated from 11 to 17 known requirements and from two to
three known `analysis_kind`s. Full backend regression suite (5247 tests)
passes. **No real-browser Playwright coverage was added this slice** —
see [IMPEDANCE_LOCUS_ANALYSIS.md](IMPEDANCE_LOCUS_ANALYSIS.md)'s own
"Known limitations" section; this is flagged honestly for owner UAT
rather than claimed as verified.

**Alternatives considered.** A second, Impedance-owned phasor
estimator/resolver call was considered and rejected — the task's own
explicit "do not create a second FFT/DFT/RMS estimator; reuse the
authoritative Phasor calculation primitives/backend service" instruction,
and `compute_phasor_diagram()` already provides exactly the Va/Vb/Vc/
Ia/Ib/Ic pairing this feature needs with zero modification. A single
shared Primary/Secondary switch covering both Voltage and Current in
Manual mode (mirroring Overcurrent's own simpler single-quantity case)
was considered and rejected, for the identical reason DEC-095's own
Phasor amendment rejected it: VT/PT and CT are physically independent
instrument transformers, and an engineer must be able to mix bases.

**Impact.** New backend modules: `app/domain/impedance.py`,
`app/services/impedance_analysis_service.py`,
`app/schemas/impedance_analysis.py`; additive-only changes to
...
Impedance Locus panel/module (markup + CSS + JS), replacing the prior
inert placeholder; zero Phasor/Overcurrent/Playback/Engineering-Context
production behavior changed. No backend files outside the new Impedance
modules and the two additive registry entries were touched. Distance
Protection, Sequence Components, and every other future analyzer remain
...
**[CLOSED] 2026-09-19 — see [DEC-098](#dec-098--analysis-browser-testruntime-hardening-shared-playback-follows-a-time-group-relabel-transparently-the-impedance-locus-cache-race-was-test-only)
for the full root-cause/fix record.** Item 1 (Phasor) was a genuine
production race, fixed in the shared Playback layer. Item 2 (Impedance)
was confirmed test-only after a full code audit found the existing
stale-response protection already correct; only the test's own
synchronization was hardened.
...
## DEC-098 — Analysis browser-test/runtime hardening: shared Playback follows a Time Group relabel transparently; the Impedance locus-cache "race" was test-only

Date: 2026-09-19
Status: Approved — implemented.
Source: owner task ("focused Analysis browser-test/runtime hardening
pass for the two known `[OPEN]` flaky Playwright races documented
during Sequence Components regression... eliminate the two known races
→ get a clean full Analysis/browser regression → preserve production
behavior unless an actual runtime race is proven").

**Decision.** Both `[OPEN]` items recorded in DEC-097's own closing
section are now closed, with different root causes and different fix
categories:

**1. Phasor Time-Group-ID reassignment — a genuine PRODUCTION race,
fixed in `frontend/index.html`.** Root cause, traced exactly (never
guessed): a Time Group's own `group_id` is ALWAYS its current origin
source's own `source_id` (`app.domain.time_grouping`: earliest
`start_time`, ties broken by `source_id` string), **recomputed fresh
from the live source set on every call, never cached/persisted** — an
explicit, already-approved design (DEC-057, unchanged, not questioned
here). Uploading a second source that happens to overlap the same
absolute-time neighborhood as an already-Playback-active Time Group can
change WHICH member is chosen as origin, silently renaming that group's
own id, even though the group's actual underlying membership (and what
an analyzer/engineer is actually tracking) has not meaningfully changed.
The shared `wwPlayback` controller had no mechanism to notice this: its
own cached `activeTimeGroupId` simply went stale, so every analyzer's
own `LoadForSelectedContext()` (`wwPlayback.activeTimeGroupId === groupId`
check) saw a mismatch and took the "claim this as a brand-new group"
branch — a full `wwPlaybackRestart()` back to `bounds.start` — discarding
the current playback position for no user-visible reason. Confirmed via
the original flaky test itself: the two committed fixtures the test
uploads (`phasor_smoke_three_phase`/`phasor_smoke_bravo_three_phase`)
share the EXACT same recorded start timestamp, so which one becomes
origin is a genuine 50/50 `source_id`-string coin flip — explaining the
observed intermittent (not permanent, not never) failure precisely.

**Fix**: a new `wwPlaybackReconcileActiveGroupIdAfterSync()`, called
once, at the ONE existing choke point every source-upload/removal/open
flow already funnels through (`wwFetchSynchronizationStateForWorkspace()`,
immediately after its own `ww.timeGroupBySourceId` cache refresh — see
that function's own long-established "ONE generic reconciliation point"
precedent comment). Since a Time Group's own `group_id` is always
itself a valid `source_id` (the origin's own, trivially a member of its
own group), re-resolving `wwPlayback.activeTimeGroupId` through the
JUST-refreshed `ww.timeGroupBySourceId` map is enough to detect and
FOLLOW a mere relabel, using data that already exists — no new API, no
new cache, no polling. A relabel updates ONLY the tracked id;
`currentTime`/`state`/`speed`/`generation` are left completely
untouched — this is never a Restart and never touches any analyzer-
specific state directly (each analyzer's own existing "already active
vs. claim fresh" check runs immediately afterward, unmodified, now
simply comparing against the corrected id). This must never be confused
with "the group no longer exists" (only a genuine source removal
produces that, which this function correctly leaves alone — see its own
code comment for the precise distinction). DEC-057's own decided origin/
tie-break rule in `app.domain.time_grouping` was **not** touched —
this is a pure frontend continuity fix for a backend behavior that was
already correctly documented and already approved, never a change to
that decision.

**Regression coverage**: a new deterministic Playwright test
(`phasor_analysis.spec.js`, "Time Group relabel (new overlapping-but-
earlier source becomes origin) never disturbs Playback continuity")
uploads a THIRD fixture (`phasor_smoke_charlie_earlier_overlap.cfg`,
new, reusing the existing `phasor_smoke_three_phase.dat` sample data)
whose own recorded start time is deliberately one full second EARLIER
than the first fixture's, while its own absolute interval still
overlaps — `(start_time, source_id)` ordering GUARANTEES the third
source becomes the new origin every single run, deterministically, with
zero dependency on `source_id` randomness (never "rely on random timing
to reproduce," the task's own explicit instruction). Verified to
actually catch the regression: temporarily disabled the fix, confirmed
the new test fails with the exact stale-id symptom, re-enabled the fix,
confirmed green again (the same verification discipline this project's
own prior regression-test additions already established). The test's
own final check accounts for the mathematically CORRECT, expected
`alignment_offset_s` shift a genuine origin change produces (derived
the same way `wwWorkspaceTimeToSourceTime()` itself would, never
independently re-derived, never asserting a naive "nothing at all
changed" expectation that would itself be mathematically wrong for a
source whose start time genuinely differs) — this hardening pass fixes
the STALE-REFERENCE bug only, never masks or changes the legitimate
coordinate-system consequence of an origin genuinely changing.

**2. Impedance locus-cache "race" — TEST-ONLY, no production defect
found.** Full code-level audit of `wwImpedanceMaybeFetchLocus()` (the
request-generation counter, the `ww.epoch`/workspace guard, and the
`locusSignature`-based short-circuit) found the EXISTING stale-response
protection already correct: a slower, superseded fetch's own captured
`myGeneration` is compared against the (possibly since-incremented)
`wwImpedanceState.locusRequestGeneration` after its own `await`
resolves, so an out-of-order-completing older request is always
discarded, never allowed to overwrite a newer result — proven by direct
inspection, not assumed (task's own explicit "do not assume it is
test-only" instruction was honored: this was an audit, not a shortcut).
The actual observed failure (a `toPass({timeout: 5000})` **timeout**,
never a wrong/corrupted value) is consistent only with "the test
observed state before the real network response had a chance to land,"
never a genuine ordering/overwrite defect — and the test's own helper
had, with no evident justification, overridden this project's own
already-configured `playwright.config.js` `expect.timeout: 10_000`
default down to a tighter `5000`.

**Fix (test-only)**: `impedance_analysis.spec.js`'s
`selectContextAndWaitForResult()` now arms a real
`page.waitForResponse()` for the actual `/impedance-locus` HTTP round
trip BEFORE the triggering `selectOption()` action (the deterministic
"relevant API response completed" condition the task itself recommends
as the preferred alternative to elapsed-time waiting), and awaits it
before returning; `waitForLocusCached()` (used by all six locus-related
tests, all of which already call it immediately after
`selectContextAndWaitForResult()`) is now left as a fast, near-instant
synchronous-render-catch-up check only, no longer racing a real network
round trip against an independently-chosen timeout at all. Verified to
genuinely tolerate real (not merely "happened to be fast enough this
run") latency: a temporary ad hoc Playwright test with a `page.route()`-
injected 6-second artificial delay on `/impedance-locus` (well beyond
the OLD 5000ms budget) passed cleanly against the new code, confirming
the fix is not merely a bigger number but a genuinely deterministic,
latency-tolerant wait.

**Stale-response protection audit (task's own section 6).** Confirmed
already present and correct for the impedance locus fetch (see above);
also spot-checked the SAME pattern in the Phasor diagram fetch, the
Sequence Components fetch, and the Impedance current-point fetch — all
four already use the identical `requestGeneration`/`ww.epoch`/
`workspaceId` triple-guard, all four already correctly discard a
superseded response. No change was needed or made to any of these; this
was verification, not remediation.

**Repeat-run validation.** Both regression tests (the new deterministic
Phasor relabel test, and the original now-fixed flaky Phasor test) were
each run 10 consecutive times in isolation: 10/10 passed both times
(20/20 total). The full Analysis Playwright suite (`phasor_analysis`,
`overcurrent_analysis`, `impedance_analysis`, `sequence_components_
analysis`, `playback`, `analysis_related_waveforms`,
`phasor_bare_context` — 168 scenarios) was run 3 complete times: 168/168
passed every run (504/504 total, zero flaky failures). Full backend
regression suite passes unchanged (no backend Python files were
touched by this pass at all). `git diff --check` clean.

**Alternatives considered.** Changing `app.domain.time_grouping`'s own
origin tie-break rule (e.g., preferring "earliest uploaded" over
"earliest `source_id` string") was considered and rejected — that is
DEC-057's own already-approved, documented design, out of this
hardening pass's authorized scope ("preserve production behavior unless
an actual runtime race is proven" — the backend's own group-derivation
logic is not itself racy, it is a pure, deterministic function of its
inputs; only the FRONTEND's failure to notice a legitimate relabel was
the actual bug). Recomputing `wwPlayback.currentTime` to compensate for
a changed coordinate origin (attempting to preserve "the same physical
instant" rather than "the same raw workspace-time number") was
considered and rejected as out of scope — a genuine Playback SEMANTICS
change the task explicitly forbids ("do not change Playback semantics
beyond fixing a proven race"), not required by the actual reported bug
(whose own real-world trigger, two fixtures sharing an identical
timestamp, produces zero coordinate drift), and not something any
analyzer's own existing code was found to need. Simply raising
`impedance_analysis.spec.js`'s own timeout to a larger fixed number was
considered and rejected as exactly the "large timeout increase" category
of non-fix the task explicitly forbids; the deterministic
`page.waitForResponse()` wait was chosen instead.

**Impact.** `frontend/index.html`: one new function
(`wwPlaybackReconcileActiveGroupIdAfterSync()`) plus one new call site
inside the existing `wwFetchSynchronizationStateForWorkspace()`; zero
existing function signatures, zero analyzer-specific files, zero
Phasor/Overcurrent/Impedance/Sequence Components calculation logic
touched. New fixture: `backend/tests/fixtures/comtrade/
phasor_smoke_charlie_earlier_overlap.cfg` (reuses the existing
`phasor_smoke_three_phase.dat`, no new `.dat` file). New/modified
Playwright: `browser-tests/phasor_analysis.spec.js` (one new
deterministic test), `browser-tests/impedance_analysis.spec.js` (two
shared helper functions hardened, zero individual test bodies changed).
No backend Python files touched. No architecture beyond the one new
Playback-continuity invariant documented here changed — DEC-057,
DEC-085, DEC-095/096/097's own shell/shared-primitive architecture are
all unchanged.
