...
day ([DECISIONS.md — DEC-095's second amendment](DECISIONS.md#dec-095--a-shared-analysis-input-source-concept-recordingmanual-is-introduced-overcurrent-gets-the-first-manual-input--calculator-mode-implementation),
[PHASOR_ANALYSIS.md — Manual Input / Calculator mode](PHASOR_ANALYSIS.md#manual-input--calculator-mode-2026-09-16-dec-095-second-amendment),
[ANALYSIS_INPUT_SOURCE.md — Phasor's own implementation](ANALYSIS_INPUT_SOURCE.md#what-phasors-own-implementation-looks-like-end-to-end)).**

Reuses the SAME shared shell Overcurrent's own architectural correction
(below) established — Input Source segmented control, the three-region
markup split (`Input Source toggle` / `#wwPhasorRecordingSection` /
always-visible `#wwPhasorBody`), the availability/auto-select logic —
implemented from day one, never the flawed intermediate "Manual coupled
to the recording empty-state" design Overcurrent briefly shipped.
Phasor's own manual value is structurally richer than Overcurrent's:
six independent roles (Va/Vb/Vc/Ia/Ib/Ic) across TWO genuinely
independent bases. **Owner's hard requirement: Voltage basis and
Current basis are separate selectors, never one shared Primary/
Secondary switch** — `wwPhasorState.manual.voltageBasis` (its own
VT/PT ratio) and `.currentBasis` (its own CT ratio) never touch each
other, proven by a dedicated mixed-basis golden test (Voltage Primary,
Current Secondary) at every layer. This extends DEC-095 with a new
general principle: a future analyzer with N independent physical
quantities should expect N independent bases in its own manual state,
not one shared switch, whenever those quantities are measured through
independent instrumentation.

**Backend.** New workspace-scoped `GET .../phasor-manual` endpoint;
new, small, GENERALIZED `convert_manual_magnitude_to_secondary()`
(`app/domain/phasor.py`, parameterized on Voltage/Current so ONE
function serves both quantities, mirroring
`convert_to_relay_secondary()`'s own two-step shape but generalized
where Overcurrent never needed to be) normalizes every entered
magnitude to its Secondary-equivalent — the chosen canonical internal
basis, matching Overcurrent's own DEC-095 precedent since Phasor's
Recording-mode result carries no basis concept to weigh against at
all. `evaluate_manual_phasor_role()` evaluates each of the six roles
fully independently: not-entered reports the EXISTING
`ROLE_STATUS_MISSING`; an invalid basis/ratio blocks every role in that
role's OWN family only (`ROLE_STATUS_NEEDS_CONFIGURATION`) — **no new
role-status vocabulary was introduced anywhere in this slice.** The
result (`ManualPhasorDiagramResult`) reuses `PhasorDiagramRoleResult`
VERBATIM for `roles`, so the SAME frontend renderer
(`wwPhasorRenderDiagramSvg()`/`wwPhasorRenderValuesList()`) draws
either Recording's or Manual's own result with zero new rendering
code — `wwPhasorActiveDiagram()` picks between two fully-separate state
fields (`wwPhasorState.latestDiagram` vs.
`wwPhasorState.manual.latestResult`) by the active `inputSource`.
Angle normalization reuses the EXISTING, previously-private
`_normalize_angle_deg()` function verbatim.

**Frontend.** Reuses Overcurrent's own segmented-control CSS classes
VERBATIM (`.ww-oc-axis-toggle-group`/`-btn`/`--active`/`-panel`/`-row`/
`-hint`), per the task's own explicit "same segmented-control language
as OC" instruction. The Manual Phasors panel groups Voltage (basis
selector, VT/PT ratio row shown only when Primary, Va/Vb/Vc rows) and
Current (same shape, CT ratio) as two visually separated sub-sections;
the 6×4 role fields (enabled/magnitude/unit/angle) are wired via one
loop over the six-role order, not 24 hand-written listeners. One
...
`wwPhasorRenderDiagramResult()` picks whichever status row belongs to
the currently active mode. Related Waveforms is hidden/collapsed
entirely in Manual mode (mirroring Overcurrent's own corrected choice);
Playback's fetch-triggering half is gated off while Manual is active
(transport-UI-sync stays unconditional, per DEC-085).

**A pre-existing OC static test needed fixing as a direct consequence**:
`test_input_source_panel_lives_outside_and_before_the_recording_section`
used an unscoped `source.index('class="panel ww-oc-input-source-panel"')`,
which now matches Phasor's own (earlier-in-file) reuse of the identical
class name instead of Overcurrent's — fixed by scoping the search to
OC's own panel boundaries. One pre-existing Phasor Playwright test's
own stale `#wwPhasorBody` visibility assertions were also updated to
match the corrected always-visible behavior (two occurrences: the
"no context selected" scenario and the "empty workspace" scenario, the
latter renamed to describe the new Manual-auto-selected behavior).

**Tests**: `backend/tests/test_phasor_domain.py`
...
(`TestComputePhasorManualDiagram`, golden + mixed-basis + per-family
isolation), `backend/tests/test_phasor_diagram_api.py`
(`TestManualPhasorDiagramEndpoint`, golden 132kV/1200A primary ->
110V/1A secondary end-to-end via real HTTP with zero prior upload),
`backend/tests/test_frontend_phasor_analysis.py`
(`TestManualInputCalculatorMode`), and a new
`browser-tests/phasor_analysis.spec.js` describe block (15 real-browser
scenarios: empty-workspace availability, the golden example with zero
recording-dependent requests, mixed basis, ratio-field conditional
visibility, kA/A and kV/V equivalence, angle normalization, partial
input, invalid-ratio family isolation, invalid-magnitude row isolation,
Related Waveforms hidden, Playback independence, Recording/Manual
state isolation, Recording-unavailable clean state, responsive
geometry at 1366px/1024px). Full Phasor Playwright suite (34
scenarios), full OC/Related-Waveforms/Playback/bare-context Playwright
suites (142 scenarios), and full backend suite all pass; two pre-
existing Phasor Playback-timing-sensitive tests reconfirmed unrelated
flakes (pass in isolation). `git diff --check` clean.

**Stop condition honored**: task instruction was "stop after this
Phasor Manual Input slice" — Impedance Locus/Sequence Components/
Distance Manual mode remain explicitly out of scope, not started; no
redesign of the whole Analysis page was attempted; no IEC/RMS/CT/VT
math was touched unnecessarily beyond the new, additive Manual-mode
functions; no persistence was introduced for Manual inputs; no fake
recordings or Engineering Contexts were created anywhere in this slice
or its tests; Recording-basis auto-detection from a recording's own
metadata remains a separate, unstarted problem, deliberately not
solved here.

## What was done in the prior session — Overcurrent architectural correction: Manual Input decoupled entirely from recordings
...
[PHASOR_ANALYSIS.md](PHASOR_ANALYSIS.md)'s own new "Bay-centric Phasor
Diagram" section).** UAT of the Bay/Quantity/Mode page found it too
restrictive — an engineer had to pick one role subset at a time, never
seeing Voltage and Current together the way Waveform already shows every
channel at once. Owner instruction: selecting a bay/context should
automatically resolve and show ALL available supported phasor inputs
together on one diagram; the engineer then hides/shows individual
vectors by clicking their labels.

**Backend**: a new orchestration function, `compute_phasor_diagram()`
(`app/services/phasor_analysis_service.py`, alongside the original,
UNCHANGED `compute_phasor_analysis()`), independently resolves all six
single-phase requirements via the unchanged `resolve_analysis_inputs()`,
maps outcomes to a new 5-value role-status vocabulary
(`available`/`missing`/`needs_configuration`/`ambiguous`/
`not_eligible`), computes one shared reference frequency, adds a NEW
cross-role `timebases_aligned()` check (the resolver's own internal
check never fires for six independently-resolved single-role
requirements), and estimates each eligible role via the unchanged
`estimate_phasor()`. Only a reference-frequency conflict or timebase
incompatibility blocks the WHOLE combined result; one bad role never
blocks any other. New endpoint `GET .../phasor-diagram` alongside the
original, unchanged `GET .../phasor`. New domain types
`PhasorDiagramResult`/`PhasorDiagramRoleResult` (`app/domain/phasor.py`,
additive) and wire shapes (`app/schemas/phasor_analysis.py`, additive).
16 new backend tests (`test_phasor_diagram_service.py` — 12,
`test_phasor_diagram_api.py` — 4), covering full/partial bays,
ambiguous-role and ineligible-role isolation, both whole-result-blocking
conditions (with an explicit-override recovery case), a compatible
multi-source context, and absolute-angle preservation across a genuine
V-I phase lag. Full existing Phasor backend suite (55 tests) passes
unchanged.

**Frontend**: Quantity/Mode selectors removed entirely — the
Engineering Context (Bay) selector is the only primary control. The old
two-call flow (input-resolution, then phasor) is replaced by ONE call to
the new aggregation endpoint. The Values list is now two sections
(Voltage/Current), each row a full row-as-toggle-button (reusing the
EXACT `#channelGroups` row-as-button convention Waveform channel
visibility already established) showing magnitude/unit/
`angle_deg_absolute` for an `available` role, or a status label for
anything else. **Visibility is a new, pure frontend display-state layer**
(`wwPhasorState.visibleRoles`) — toggling a row re-renders locally from
the last-fetched result with ZERO new backend requests; it persists
across an Analysis Time change and resets only when the selected context
itself changes. The diagram now draws up to six vectors: geometry always
uses `angle_deg_absolute` (Voltage and Current are NEVER independently
zero-referenced, preserving the true V-I angular relationship — the
single most safety-critical constraint of this redesign), Voltage and
Current use SEPARATE graphical magnitude scales (never one raw shared
radius), with a transparent scale-ratio annotation shown whenever both
families are present, and Current vectors are drawn dashed against
Voltage's solid, both still colored by phase. `test_frontend_
phasor_analysis.py` revised in place (obsolete Quantity/Mode/
resolution-list assertions replaced; new classes for bay-centric
aggregation, visibility state, and dual-scale-diagram invariants — 70
tests total, all passing). `phasor_analysis.spec.js` rewritten: a
12-step bay-centric scenario (all six roles load together, hide/show
individual vectors with zero additional requests, visibility survives an
Analysis Time change, visibility resets on a genuine context switch)
plus a rewritten partial-bay scenario; the bootstrap suite is otherwise
unchanged. Full frontend static suite and full Playwright suite both
pass (one pre-existing, unrelated `smoke.spec.js` timing flake
reproduced as passing on isolated re-run).

**Explicitly deferred**: Playback integration (still out of scope — the
owner's own closing instruction for this task was "do NOT proceed
automatically to Playback"), Per-Unit display, neutral-phasor roles
(`Vn`/`In`), sequence components, impedance/distance, automatic
cross-source context merging, and a manual raw-channel picker (still
never introduced).
...
## What was done in the prior session — Phasor UAT fix: generic nav tooltip corrected + automatic Engineering Context suggestion bootstrap
...
confirmed `[FACT]` on 2026-08-22 but explicitly scoped as "unaffected"
at the time (that pass only fixed the Measurement Group path):
`app.domain.per_unit.resolve_per_unit()`'s VOLTAGE branch used to
divide a measured channel directly by the raw entered
`voltage_base_value`, regardless of the channel's own effective
reference. Root cause verified by direct code reading (Phase 1, before
any change) -- confirmed via `grep`, not assumed.

**Corrected semantics** (now identical to
`app.domain.voltage_group_config.VoltageBaseConfiguration.nominal_voltage_ll_kv`'s
own long-established meaning): `voltage_base_value` is uniformly the
...
A resolved reference is now REQUIRED for VOLTAGE to resolve `configured`
at all (new `voltage_reference_undetermined` gate, mirroring the
Measurement Group resolver's own long-standing "never silently guess"
behaviour -- previously only the CURRENT branch had this requirement).

**Current base is explicitly unchanged in its own governing formula**
(`Ibase = Sbase / (sqrt(3) x Vbase_LL)`, always the raw nominal
`Vbase_LL`, regardless of reference) -- the old `voltage_base_ll_volts()`
helper (which multiplied by `sqrt(3)` for a line-to-ground reference)
is removed outright, since under the corrected field semantics that
conversion is no longer meaningful and would have silently double-
converted. Verified directly: `Ibase = Sbase / (sqrt(3) x 275 kV)` for
a 1000 MVA/275 kV derived Current Base now gives the SAME ~2.0995 kA
regardless of reference (matches the Measurement Group worked example
exactly), where it previously gave a DIFFERENT (also-wrong) number for
a line-to-ground reference.
...
(the owner's own reported defect: 275 kV entered, ~159 kV measured
previously read ~0.577 pu; now correctly reads ~1.0 pu) -- deliberate,
not preserved for backward compatibility. Line-to-line configurations,
direct/manual current-base mode, Measurement Group arithmetic, DEC-051
precedence, and DEC-052 inheritance are all byte-for-byte unchanged
(verified directly). Calculated channels that fall back to Source
Default inherit the fix automatically -- no separate correction, since
they call the identical, now-corrected `resolve_per_unit()`. Slice 2
coverage and Slice 3 provenance reflect the corrected arithmetic with
zero code changes of their own.

**Frontend**: the Source Default modal's own cosmetic "Calculated
Current Base" preview had the identical pre-fix `sqrt(3)`-for-LG
multiplication baked in (`wwPerUnitEditorResolvedCurrentBaseText()`) --
corrected to match, or it would have silently mismatched what the
backend actually saves. Its Voltage Reference tooltip previously
claimed the reference is "never applied automatically to a displayed
channel's own per-unit value" -- true before this fix, false after --
corrected. A new secondary preview line ("Nominal system voltage: 275
kV L-L — Effective L-G base: 158.77 kV"-shaped) was added per the
task's own optional UI-clarity suggestion, shown once both the base and
reference are known, in both the manual-override and auto-detected
branches.

**Files changed**: `backend/app/domain/per_unit.py` (the fix;
`voltage_base_ll_volts()` removed), `frontend/index.html` (preview/
tooltip corrections), `backend/tests/test_per_unit_source_default_voltage_reference.py`
(new, 18 tests -- the full mandated numerical scenario set),
`backend/tests/test_frontend_per_unit_source_default_voltage_reference.py`
(new, 10 tests), plus `test_per_unit_domain.py`,
`test_per_unit_display_endpoints.py`, `test_measured_unit_per_unit.py`,
`test_per_unit_provenance_service.py`, `test_per_unit_provenance_api.py`,
`test_per_unit_coverage_api.py` updated wherever they had locked in the
old, now-incorrect arithmetic -- each with an explicit comment
explaining the corrected engineering semantics, never a silently
weakened assertion.

**Tests**: full per-unit + frontend test sweep passes; full backend
regression suite passes with zero failures (exit code 0); `git diff
--check` clean.
