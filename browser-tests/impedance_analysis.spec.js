...
async function selectContextAndWaitForResult(page, contextId) {
  ...
  if ((await select.inputValue()) !== contextId) {
    ...
    // DEC-114 (2026-09-24): backend `compute_impedance_locus()` no longer
    ...
    // matching `wwImpedanceState.locusSignature`) before this explicit
    // reselect even runs, so `wwImpedanceMaybeFetchLocus()`'s own
    // signature short-circuit correctly skips issuing a SECOND, redundant
    // `/impedance-locus` request -- the exact caching discipline this
    // suite already verifies elsewhere (e.g. "no full-locus or waveform
    // refetch occurs during Playback ticks"). A strict `waitForResponse`
    // here would wait forever for a request that correct behavior
    // legitimately never sends. Race a real response against the locus
    // state itself having been (re)populated since this function started
    // -- covers a genuinely new fetch AND an already-cached hit, and the
    // "changed since snapshot" comparison (not just "non-empty") avoids
    // a false-positive if a PRIOR, different context already left a
    // non-empty `locusPoints` array in place.
    const beforeSnapshot = await page.evaluate(() => JSON.stringify(wwImpedanceState.locusPoints));
    const locusResponse = page.waitForResponse((r) => r.url().includes("/impedance-locus"), { timeout: 20000 }).catch(() => null);
    await select.selectOption(contextId);
    await Promise.race([
      locusResponse,
      expect(async () => {
        const current = await page.evaluate(() => JSON.stringify(wwImpedanceState.locusPoints));
