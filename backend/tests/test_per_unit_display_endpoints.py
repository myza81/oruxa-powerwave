...
def _upload(client, workspace_id, comtrade_fixtures_dir, stem="synth_ascii"):
    ...
    Group discovery. This module's own tests exercise the SOURCE-DEFAULT
    (group-less) per-unit path specifically, which a group-aware
    resolution takes precedence over per DEC-051 -- clear whatever
    discovery created immediately after upload so every test below
    keeps exercising the source-default path unaffected."""
...
def _upload_without_clearing(client, workspace_id, comtrade_fixtures_dir, stem="synth_ascii"):
