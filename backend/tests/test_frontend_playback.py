...
class TestPlaybackIsNotATopLevelPage:
    ...
    def test_analysis_menu_exists_but_is_not_a_playback_destination(self):
        ...
        The destination's `id` (`mainNavAnalysisBtn`) and its underlying
        page (`#pageAnalysis`, `<h2>Analysis</h2>`) are unchanged; see
        test_frontend_phasor_analysis.py's own `TestAnalysisMenuExists`
        for the full, current assertions on the nav button's exact text."""
        source = _source()
...
class TestSharedAnalysisPlaybackStyling:
    ...
    def test_mount_functions_flatten_the_shared_transport_wrapper(self):
        """Now FOUR occurrences: the shared CSS declaration plus three
        mount functions (Phasor, Overcurrent, and Impedance Locus v1 --
        see docs/project-memory/IMPEDANCE_LOCUS_ANALYSIS.md)."""
        source = _source()
        assert source.count("ww-analysis-playback-transport") == 4
