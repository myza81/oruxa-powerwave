...
// used to exercise it changed. Owner UAT (2026-09-26, DEC-085 update):
// the Playback Cursor VISUALIZATION is Analysis-owned -- the Waveform
// Time Group canvas's `.ww-tg-playback-cursor-overlay` stays mounted but
// is never rendered, so tests below that return to Waveform assert it
// stays HIDDEN while the shared clock itself carries over (full owner
// reproduction coverage: playback_waveform_ownership.spec.js).
//
...
// Time Group and gives access to the Waveform canvas, whose dormant
// Playback Cursor overlay must stay hidden), create a single-phase Engineering Context
...
test.describe("Event Playback Slice 1", () => {
  ...
  test("Waveform Time Group toolbar no longer mounts Playback controls", async ({ page }) => {
    ...
    // The Playback Cursor overlay DOM stays mounted but dormant: the
    // Playback Cursor visualization is Analysis-owned (DEC-085 update,
    // 2026-09-26).
  ...
  // ---- Test 1: Play moves ----
  test("Play advances the current-time readout; Waveform shows no playback cursor", async ({ page }) => {
    ...
    // Owner UAT (2026-09-26, DEC-085 update): switching from Analysis
    // back to Waveform mid-playback must NOT show the green Playback
    // Cursor -- the visualization is Analysis-owned -- while the shared
    // clock keeps running.
  ...
  test("Playing a second Time Group stops the first", async ({ page }) => {
    ...
    // (dormant) Playback Cursor overlay stays hidden.
...
test.describe("Event Playback Slice 2", () => {
  ...
  test("Seek moves the readout/cursor substantially and playback continues from there", async ({ page }) => {
    ...
    // Owner UAT (2026-09-26, DEC-085 update): the sought/playing time
    // never paints a Playback Cursor on Waveform.
    await page.locator("#mainNavWaveformBtn").click();
    await expect(canvas).toBeVisible();
    await expect(canvas.locator(".ww-tg-playback-cursor-overlay")).toBeHidden();
  });
